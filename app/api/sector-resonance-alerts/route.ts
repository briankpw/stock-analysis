/**
 * CRUD for per-market-segment 6-Signal Resonance alerts.
 *
 * Structural mirror of `/api/resonance-alerts` — the two features
 * share the same "daily digest + on-change" schema but persist to
 * separate tables (see `sector_resonance_alerts` in `lib/db.ts` v10).
 *
...
import {
  deleteSectorResonanceAlert,
  listSectorResonanceAlerts,
  upsertSectorResonanceAlert,
...
export async function GET() {
  return NextResponse.json({ alerts: listSectorResonanceAlerts() });
}

export async function POST(req: Request) {
  try {
    const parsed = upsertSchema.parse(await req.json());
    const alert = upsertSectorResonanceAlert({
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    const ok = deleteSectorResonanceAlert(segmentId);
